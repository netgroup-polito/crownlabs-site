...
sendVote = async () => {
    ...
    let resultVotation = await fetch("https://api.apispreadsheets.com/data/10703/", {
        method: "POST",
        body: JSON.stringify({"data": {"vote":voted,"userIP":userIP,"date":date,"comment":comment_text, "groupmates": choices[0].checked ? "Yes" : "", "professor": choices[1].checked ? "Yes" : "", "admin": choices[2].checked ? "Yes" : ""}})
    })
    ...
    if (resultVotation.status === 201) {
        ...
        const userVote = {
            ...
            'groupmates': choices[0].checked ? "Yes" : "",
            'professor': choices[1].checked ? "Yes" : "",
            'admin': choices[2].checked ? "Yes" : ""
        }
